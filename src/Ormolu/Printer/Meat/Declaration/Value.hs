...
p_stmt' placer render = \case
  ...
  BindStmt NoExt (XPat l) f _ _ -> do
    located l p_pat
    space
    txt "<-"
    let placement =
  ...
  BindStmt NoExt _ _ _ _ -> error "p_stmt': expected BindStmt _ (XPat _)"
...
p_hsExpr' s = \case
  ...
  HsProc NoExt _ _ -> error "p_hsExpr' HsProc: expected XPat"
  HsStatic _ e -> do
    txt "static"
    breakpoint
    inci (located e p_hsExpr)
